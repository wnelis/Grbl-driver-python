...
control method, which can be either 'send-response' or 'character counting'. For
...
state within the driver are out of sync. Once an out-of-sync is detected, it is
...
If the check mode is disabled (toggled off), a soft reset is performed by the
Grbl device and thus transmission must be suspended until the welcome message is
received.
